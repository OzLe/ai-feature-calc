...
function exportCSV(cfg, result, cfgB, resultB) {
  ...
  downloadFile(csv, "llm-cost-analysis.csv", "text/csv");
}
...
function exportJSON(cfg, result, cfgB, resultB) {
  const data = {
    scenarioA: {
      configuration: cfg,
  ...
  downloadFile(JSON.stringify(data, null, 2), "llm-cost-analysis.json", "application/json");
}
  ...
  return (
    <div className="rounded-xl border border-cyan-500/30 bg-cyan-500/5 p-3 space-y-1.5">
      ...
      <CompactSlider label="MAU" value={cfgB.mau} min={100} max={500000} step={500}
        onChange={v => setCfgB(prev => ({ ...prev, mau: v, dau: Math.min(prev.dau, v) }))} display={fmtTokens(cfgB.mau)} t={t} />
      <CompactSlider label="DAU" value={cfgB.dau} min={10} max={cfgB.mau} step={100}
...
export default function LLMCostFramework() {
  ...
  useEffect(() => {
    ...
    if (params.has("modelId")) {
      const numericFields = ["systemPrompt", "ragContext", "userInput", "output", "agenticMult", "historyDepth", "avgTurnsHistory", "mau", "dau", "sessionsPerUser", "interactionsPerSession", "cacheRatio", "cacheReuses", "reasoningMultiplier"];
      const restored = {};
      for (const [k, v] of params.entries()) {
        restored[k] = numericFields.includes(k) ? Number(v) : v;
      }
      setCfg(prev => ({ ...prev, ...restored }));
      ...
      if (autoSaved && autoSaved.cfg && autoSaved.cfg.modelId) {
        setCfg(prev => ({ ...prev, ...autoSaved.cfg }));
        ...
        <div className="max-w-5xl mx-auto px-3 sm:px-6 py-3 sm:py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h1 className={`text-lg sm:text-xl font-bold ${t.text} mt-0.5`}>AI Feature Cost Estimator</h1>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className={`rounded-xl border ${t.border} ${t.bgCardSoft} p-4 space-y-4`}>
                <SliderField label="Monthly Active Users (MAU)"
                  ...
                  onChange={v => { setCfg(prev => ({ ...prev, mau: v, dau: Math.min(prev.dau, v) })); }} t={t} />
                <SliderField label="Daily Active Users (DAU)"
                  value={cfg.dau} min={10} max={cfg.mau} step={100}
          ...
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button onClick={() => exportCSV(cfg, result, cfgB, resultB)} className={`px-3 py-1.5 rounded-lg border ${t.border} ${t.textMuted} text-xs ${t.bgCardHoverSolid} transition-colors min-h-[44px] sm:min-h-0`}>
                  Export CSV
                </button>
                <button onClick={() => exportJSON(cfg, result, cfgB, resultB)} className={`px-3 py-1.5 rounded-lg border ${t.border} ${t.textMuted} text-xs ${t.bgCardHoverSolid} transition-colors min-h-[44px] sm:min-h-0`}>
                  Export JSON
                </button>
