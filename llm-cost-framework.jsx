...
function exportCSV(cfg, result, cfgB, resultB, appName) {
  ...
  if (appName) rows.push(["App Name", appName]);
  if (cfgB && resultB) {
...
function exportMarkdown(cfg, result, cfgB, resultB, appName) {
  const lines = [];
  lines.push(`# ${appName || "LLM Cost Analysis"}`);
  lines.push(``);
  ...
  useEffect(() => {
    ...
    if (params.has("modelId")) {
      ...
      for (const [k, v] of params.entries()) {
        if (k === "appName") continue; // handled separately
        restored[k] = numericFields.includes(k) ? Number(v) : v;
      }
      if (params.has("appName")) setAppName(params.get("appName"));
